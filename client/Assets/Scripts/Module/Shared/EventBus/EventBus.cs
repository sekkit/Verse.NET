    ...
    public class EventBus : IEventBus {
        ...
        public void Subscribe(object subscriber, string eventName, Delegate callback) {
            lock (threadLock) {
                ...
                if (replacedDelegate != null) { Log.Warning("Existing subscriber was replaced for event=" + eventName); }
            }
        }
            ...
            lock (threadLock) {
                ...
                if (!dictForEventName.IsNullOrEmpty()) {
                    ...
                    return subscriberDelegates.Map(subscriberDelegate => {
                        ...
                        } catch (Exception e) { if (throwPublishErrors) { throw; } else { Log.Error(e); } }
                        return null;
                    });
                }
                return new List<object>();
        ...
        public bool Unsubscribe(object subscriber, string eventName) {
            ...
            Log.Warning("Could not unsubscribe subscriber=" + subscriber + " from event '" + eventName + "'");
            return false;
        }
