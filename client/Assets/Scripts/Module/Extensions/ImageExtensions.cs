...
namespace Module.Extensions {
    ...
    public static class ImageExtensions {
        ...
        public static async Task<ImageResult> LoadAndDispose(Stream stream) {
            ...
            Assert.AreNotEqual(0, stream.Length, "LoadAndDispose: stream.Length");
        ...
        public static async Task<Texture2D> ToTexture2D(byte[] downloadedBytes) {
            return ToTexture2D(await LoadAndDispose(new MemoryStream(downloadedBytes)));
        }

        public static Texture2D ToTexture2D(this ImageResult self) {
            Assert.AreEqual(8, self.BitsPerChannel);
        ...
        public static Texture2D CaptureScreenshot(this Camera[] cameras, int width = 0, int height = 0) {
            ...
            if (height == 0) { height = width * Screen.height / Screen.width; }
